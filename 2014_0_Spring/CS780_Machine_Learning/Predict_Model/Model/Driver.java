...
public class Driver {
	public static void main(String[] args) {
		 ...
		 */
		String[][] raw_Data = ReadFile_StoreDataIntoArray(args, 331, 27);
		/**
		 * convert the String to double, store the data into double array.
		 */
		double[][] data = new double[331][27];
		for (int i = 0; i < data.length; i++) {
			for (int j = 0; j < data[0].length; j++) {
				data[i][j] = Double.parseDouble( raw_Data[i][j] );
			}
		}
//		/**
...
//		}
		 ...
		 */
		/**
		 * Initialize the weight for every data point and Randomly generate the double value later.
		 * Initialize the other miscellaneous variables.
		 */
		double[][] weight_bottom = new double[3][26];
		double[] weight_top = new double[3];
		double[] hidden_Layer = new double[3];
		double[][] trainingSet168 = new double[264][26];
		double[][] trainingSet167 = new double[167][26];
		...
		//5-fold
		/**
		 * u = 41, 83, 125, 167, 209(208) ;//for 209 data.
		 * u = 65, 131, 197, 263, 330;//for 331 data.
		 */
